...
void EPD_2IN9_Update_Image()
{
    ...
    switch (step) {
        ...
        case EPD_CMD_BW: { // write BW ram
            ...
            if (top == 3) {
                EPD_2IN9_WriteRam(epd_buffer, EPD_WIDTH, 16, 0, (top*128/8), 0);
            } else {
                EPD_2IN9_WriteRam(epd_buffer, EPD_WIDTH, 128, 0, (top*128/8), 0);
            }
            break;
        }
        ...
        case EPD_CMD_RED: { // write Red ram
            uint8_t top = epd_step_data[0];
            if (top == 3) {
                EPD_2IN9_WriteRam(epd_buffer, EPD_WIDTH, 16, 0, (top*128/8), 1);
            } else {
                EPD_2IN9_WriteRam(epd_buffer, EPD_WIDTH, 128, 0, (top*128/8), 1);
            }
            break;
        }
